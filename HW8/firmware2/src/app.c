            ...
            else if( appData.hidDataReceived )
            {
                ...
                switch(appData.receiveDataBuffer[0])
                {
                    case 0x80:
                        ...
                        BSP_LEDToggle( 7 );
                        BSP_LEDToggle( 15 );
                        char str[100]; //200 is max number of characters when starting from (0,0)
                        int row = receiveDataBuffer[2];
                        int i;
                        //sprintf(str, "%s", 99, receiveDataBuffer[]);
                        for (i = 0; i < 100; i++)
                            str[i] = receiveDataBuffer[4+i];
                        display_str(str, row, 0);
                    ...
                    case 0x81:
                        ...
                        if(appData.hidDataTransmitted)
                        {
                            ...
                            /*
                            if( BSP_SwitchStateGet(13) == BSP_SWITCH_STATE_PRESSED )
                            {
                                appData.transmitDataBuffer[1] = 0x00;
                            }
                            else
...
void display_str(char acter[], int sRow, int sCol) {
    ...
    // Allocated before loops
    int ind;
    int bitVal;

    for (iChar = 0; iChar < len; iChar++) {
        ind = (int) acter[iChar] - 0x20;
        for (iCol = 0; iCol < 5; iCol++) {
            for (iRow = 0; iRow < 8; iRow++) {
                bitVal = getBit(ind, iRow, iCol);
                if (bitVal) {
                    display_pixel_set(sRow+iRow+row*8,sCol+iCol+col*5,1);
                }
                else {
                    display_pixel_set(sRow+iRow+row*8,sCol+iCol+col*5,0);
                }
            }
        }
        col++;
        if (col == 25) { row++; col = 0; } //next row
    }
    display_draw();
}

int getBit(int index, int row, int col) {
    return (ASCII[index][col] & (1 << (row-1))) >> (row-1);
}
